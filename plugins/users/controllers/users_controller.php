...
class UsersController extends AppController {
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('logout', 'reset', 'verify', 'register');
    }
  ...
  /*function register() {
    ...
    function register() {
        if(!empty($this->data['User']['email'])) {
            ...
            if(empty($username)) {
                $this->Session->setFlash('Please enter an username.');
                $this->set('error',array('username_missing' => true));
                $this->render(); exit;
            }

            $this->User->recursive = -1;
            if (!$this->User->findCount(array('User.username' => $username), -1) || !$this->User->findCount(array('User.email' => $email), -1)) {
                $this->User->create();
                print_r($this->data);
                if($this->User->save($this->data)) {
                    ...
                    $content[] = 'A request to create an account on mashupkeyword.com has been submitted.';
                    $content[] = 'Please visit the following url to have your account saved';
                    //fixme: generate token
                    $content[] = Router::url('/users/verify/save/'.$user['User']['email_token'], true);
                ...
                $this->User->invalidate('email', 'The account or the e-mail you entered is already in use');
            }
        }
    }
